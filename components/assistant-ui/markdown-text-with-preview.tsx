...
const CodeIcon: FC<{ className?: string }> = ({ className }) => (
  ...
  >
    ...
    <line x1="11" y1="17" x2="13" y2="7" />
    <polyline points="16 7 20 12 16 17" />
  </svg>
);

const PreviewableMarkdownTextImpl = () => {
  return (
    <MarkdownTextPrimitive
...
export const PreviewableMarkdownText = memo(PreviewableMarkdownTextImpl);
  ...
  return (
    <div className="my-3 overflow-hidden rounded-xl border">
      <div className="border-border/50 bg-muted/50 flex items-center justify-between border-b px-3.5 py-1.5 text-xs">
        <span className="text-muted-foreground flex items-center gap-1.5 font-medium lowercase">
        ...
        <div className="flex items-center gap-1">
          {previewable && (
            <TooltipIconButton
              tooltip={showPreview ? "Show code" : "Preview"}
      ...
      {showPreview ? (
        <iframe
          srcDoc={code}
          className="bg-background h-96 w-full"
          sandbox="allow-scripts"
          title="preview"
        />
      ) : (
        <pre className="bg-muted/30 overflow-x-auto p-3.5 text-[13px] leading-relaxed">
...
const previewComponents = baseComponents;
