...
如果输出较长，宁可输出更冗长的完整代码，也不要截断、省略或只生成部分结构。如果受模型长度限制无法一次性完整输出，应优先保证整体结构完整，并在说明中告知用户哪些部分被省略。
...
如果 HTML 中包含图片 URL、SVG、icon class、背景图 URL，必须直接使用这些 URL。外部 SVG 资源，例如 img 标签的 src="... .svg"，应直接使用 img 标签引用，禁止将其重写为内联 SVG；禁止以“防止资源失效”“自包含”“fallback”“不确定外部资源是否与截图一致”等理由替换提供的 URL。

唯一例外：URL 明确返回 404 或无法访问时，才可以生成占位元素，并在说明中标注。
